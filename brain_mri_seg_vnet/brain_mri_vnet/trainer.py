...
class Trainer:
    """Trainer class trains brain MRI segmentation with VNET"""
    ...
    def _train(self):
        ...
        for i, (x, y) in batch_iter:
            ...
            # print("input", input_x.shape, input_x.dtype, "target", target_y.shape, target_y.dtype)
            ...
            if self.writer is not None:
                self.writer.add_images(
                    "Train/masks/true",
                    target_y.permute(1, 0, 2, 3),
                    len(self.training_loss),
                )
                self.writer.add_images(
                    "Train/masks/pred",
                    torch.argmax(out, dim=1).permute(1, 0, 2, 3),
                    len(self.training_loss),
                )

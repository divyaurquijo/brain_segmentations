...
def segmentation_target(directory: str, directory_path: str, images: list, mask: list):
    """Call segmentation for each images and target masks and call save_gif to save each 3D images

    Args:
        directory (str): name of directory
        directory_path (str): path to directory
        images (list): input images
        mask (list): target masks
    ...
    filepath_out = f"{directory_path}/Segmentation/segmentation_target_{directory}.gif"
    save_gif(segmentations_target, filepath_out)
    return Path(filepath_out)


def segmentation_pred(directory: str, directory_path: str, images: list, mask: list):
    ...
    filepath_out = (
        f"{directory_path}/Segmentation/segmentation_prediction_{directory}.gif"
    )
    save_gif(segmentations_target, filepath_out)
    return Path(filepath_out)

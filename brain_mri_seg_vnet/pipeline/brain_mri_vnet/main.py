...
@dsl.pipeline(
    name="brain-mri-vnet",
    description="Pipeline for Brain MRI VNET",
    pipeline_root="gs://vnet_pipeline",
)
def pipeline(
    ...
    op_evaluation = (
        evaluation(
            model_config_path,
            op_preprocess_dataset.outputs["test_dataloader"],
            op_train.outputs["model_weights"],
        )
        .add_node_selector_constraint(
            "cloud.google.com/gke-accelerator", "NVIDIA_TESLA_K80"
        )
...
if __name__ == "__main__":
    compiler.Compiler().compile(
        pipeline_func=pipeline, package_path="./pipeline/vnet_pipeline.json"
    )

    job = aip.PipelineJob(
        display_name="brain-mri-vnet",
        template_path="./pipeline/vnet_pipeline.json",
        pipeline_root="gs://vnet_pipeline",
